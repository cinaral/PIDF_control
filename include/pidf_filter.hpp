#ifndef PIDF_FILTER_HPP_CINARAL_220822_0805
#define PIDF_FILTER_HPP_CINARAL_220822_0805

#include "types.hpp"

namespace pidf_filter
{
...
{
	const real_t coef_x = -2 / (2 * T_f + T_s);
	const real_t coef_x_next = 2 / (2 * T_f + T_s);
	const real_t coef_y = -(2 * T_f - T_s) / (2 * T_f + T_s);
    ...
    const real_t y[], real_t y_next[])
{
	...
	for (uint_t i = 0; i < Y_DIM; ++i) {
		const real_t coef_x = -(K_p[i] * (2. * T_f - T_s) + 2. * K_d[i]) / (2. * T_f + T_s);
		const real_t coef_x_next = (K_p[i] * (2. * T_f + T_s) + 2. * K_d[i]) / (2. * T_f + T_s);
     ...
     real_t y_next[])
{
	for (uint_t i = 0; i < Y_DIM; ++i) {
		const real_t coef_x_prev =
		    (2. * K_p[i] * (2. * T_f - T_s) + K_i[i] * T_s * (T_s - 2. * T_f) + 4. * K_d[i]) /
		    (2. * (2. * T_f + T_s));
		const real_t coef_x =
		    (-8. * K_p[i] * T_f + 2. * K_i[i] * T_s * T_s - 8. * K_d[i]) / (2. * (2. * T_f + T_s));
		const real_t coef_x_next =
		    (2. * K_p[i] * (2. * T_f + T_s) + K_i[i] * T_s * (T_s + 2. * T_f) + 4. * K_d[i]) /
		    (2. * (2. * T_f + T_s));
		const real_t coef_y_prev = (2. * T_f - T_s) / (2. * T_f + T_s);
		const real_t coef_y = (-4. * T_f) / (2. * T_f + T_s);
