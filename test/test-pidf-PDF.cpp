...
//* dt__x = A*x + B*x
void
control_fun(const real_t x[], const uint_t i, real_t u_next[])
{
	real_t error_next[u_dim];
	error_next[0] = R - x[0];
...
main()
{
	...
	matrix_rw::read<t_dim, u_dim>(test_dat_prefix + x_arr_chk_fname, u_arr_chk);
	...
	for (uint_t i = 0; i < t_dim; ++i) {
		...
		for (uint_t j = 0; j < x_dim; ++j) {
			real_t error = 0; // std::abs(x_[j] - x_chk_[j]);
			if (error > max_error) {
				max_error = error;
			}
		}
